...
function setup(){
  ...
  solo = createSprite(200,height*0.80,400,20);
  solo.addImage("ground",imagemSolo);
  solo.x = solo.width / 2;
  
  //Criar Sprite do Solo Invisível
  soloInvisivel = createSprite(200,height*0.85,400,20);
  ...
  fimDeJogo = createSprite(300,height-200);
  fimDeJogo.addImage("fimDeJogo",ImgFimDeJogo);
  fimDeJogo.scale = 0.5;
  fimDeJogo.visible = false;
  
  reiniciar = createSprite(300,height-150);
...
function draw(){
  ...
  text("Pontuação: " + pontuacao, 500, height-270);
...
function gerarNuvens(){
  ...
  if(frameCount % 60 === 0){
    var nuvem = createSprite(600,height-70,40,10);
    nuvem.velocityX = -3;
    ...
    nuvem.y = Math.round(random(300,450));
...
function gerarObstaculos(){
  if(frameCount % 60 === 0){
    var obstaculo = createSprite(width,height*0.8,10,40);
